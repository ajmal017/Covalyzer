...
class DownloadApp(EClient, wrapper.EWrapper):
    ...
    def headTimestamp(self, reqId: int, headTimestamp: str) -> None:
        ...
        globvars.logger.info("Head Timestamp for %s is %s", contract, ts)
        if ts > self.args.start_date or self.args.max_days:
            globvars.logger.warning("Overriding start date, setting to %s", ts)
            self.args.start_date = ts  # TODO make this per contract
        if ts > self.args.end_date:
            globvars.logger.warning("Data for %s is not available before %s", contract, ts)
            self.done = True
            return
    ...
    def historicalDataEnd(self, reqId: int, start: str, end: str) -> None:
        ...
        if len(self.pending_ends) == 0:
            globvars.logger.info("All requests for %s complete",self.current)
            # resample(False, make_download_path(self.args, contract))
        ...
        if self.done == True:
            self.disconnect()
            globvars.logger.info(f"finished")
            exit(0)


    @iswrapper
    def connectAck(self):
        globvars.logger.info("Connected")
    ...
    def start(self):
        if self.started:
            return
        # globvars.logger.info("reset errorcode")
        # globvars.downerrorcode["errorcode"] = 0
        # globvars.downerrorcode["contract"] = None
        # globvars.downerrorcode["dwnpath"] = None
    ...
    def error(self, req_id: TickerId, error_code: int, error: str):
        ...
        contract = self.requests.get(req_id)
        globvars.downerrorcode["errorcode"] = error_code
        globvars.downerrorcode["contract"] = contract
        # globvars.downerrorcode["dwnpath"] = dwnpath

        if str(req_id) != "-1":
            globvars.logger.error("ID %s Code %s Msg: %s",str(req_id),str(error_code),str(error))
            # dwnpath = make_download_path(self.args, contract)
