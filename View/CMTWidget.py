...
class CMTWidget(QWidget):
    def __init__(self, model, controller, *args):
        ...
        self.charttype = "TimeValue"

        self.c = 0
        self.r = 0


    def setChartType(self,c):
        self.charttype = c
    ...
    def onSectionDoubleClicked(self, logicalIndex):
        ...
        globvars.logger("onSectionDoubleClicked: %s", str(logicalIndex))
    ...
    def doubleClickedCell(self, idx):
        ...
        globvars.logger.info("clicked on %s %i", str(sym), int(c))
        if c == 1:
            a = self.controller.getStockData(cc)
            self.positionViewer.updateMplChart(cc, a, self.charttype)
