...
class PositionViewer(QWidget):
    def __init__(self):
        ...
        # self.ax2.set_ylabel("STOCK")
        # self.ax.set_ylabel("OPTION - TIMEVALUE")
        qvb = QVBoxLayout()
        qvb.addWidget(self.sc)
        self.setLayout(qvb)

    def updateMplChart(self, cc, alns, datatype2ndyaxis):
        self.ax.clear()
        self.ax2.clear()
        ...
        if dfsk is not None:
            ...
            comb['optionprice'] = comb.apply(lambda row: self.calc_optionprice(cc,row), axis=1)
            ...
            if datatype2ndyaxis == "TimeValue":
                apdict = mpf.make_addplot(comb['timevalue'], ax=self.ax, color='black')
                self.ax.set_ylabel("TIMEVALUE")
            else:
                apdict = mpf.make_addplot(comb['optionprice'], ax=self.ax, color='black')
                self.ax.set_ylabel("OPTIONPRICE")
    ...
    def calc_optionprice(self, cc, row):
        oval = row['OClose']
        return oval
